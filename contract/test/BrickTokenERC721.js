...
const BrickToken = artifacts.require('BrickTokenERC721');
 ...
 */
contract('BrickTokenERC721', function (accounts) {
  it('should assert true', async function () {
    await BrickToken.deployed();
...
//       instance.propose.sendTransaction(accounts[0], '', '', {
//         from: accounts[0],
//       }),
...
//     const instance = await WeddingToken.deployed();
//     await truffleAssert.reverts(
//       instance.updateProposition.sendTransaction(accounts[1], '', '', {
//         from: accounts[0],
//       }),
...
//     const instance = await WeddingToken.deployed();
//     await truffleAssert.reverts(
//       instance.acceptProposition.sendTransaction(accounts[1], '0x', '0x', {
//         from: accounts[0],
//       }),
//       "WeddingToken: proposition doesn't exist",
//     );
...
//     const instance = await WeddingToken.deployed();
//     await truffleAssert.reverts(
//       instance.requestDivorce.sendTransaction({ from: accounts[0] }),
...
//     const instance = await WeddingToken.deployed();
//     await truffleAssert.reverts(
//       instance.confirmDivorce.sendTransaction({ from: accounts[0] }),
//       'WeddingToken: not in marriage',
//     );
//   });
...
//     const instance = await WeddingToken.deployed();
//     const tx = await instance.propose.sendTransaction(
//       accounts[1],
//       'some link',
//       'some data',
//       { from: accounts[0] },
...
//       return (
//         ev.from.toLowerCase() === accounts[0].toLowerCase() &&
//         ev.to.toLowerCase() === accounts[1].toLowerCase() &&
//         new BN(ev.id).eq(new BN(1)) &&
//         ev.metaUri === 'some link' &&
//         ev.condData === 'some data' &&
//         ev.authorAccepted &&
//         !ev.receiverAccepted
//       );
//     });
...
//       return (
//         ev.from.toLowerCase() === accounts[0].toLowerCase() &&
//         ev.to.toLowerCase() === accounts[2].toLowerCase() &&
//         new BN(ev.id).eq(new BN(2)) &&
//         ev.metaUri === 'some link' &&
//         ev.condData === 'some data' &&
//         ev.authorAccepted &&
//         !ev.receiverAccepted
...
//     const instance = await WeddingToken.deployed();
//     await truffleAssert.reverts(
//       instance.propose.sendTransaction(accounts[1], '', '', {
//         from: accounts[0],
//       }),
...
//     const instance = await WeddingToken.deployed();
//     const tx = await instance.updateProposition.sendTransaction(
//       accounts[1],
//       'new link',
//       'new data',
//       { from: accounts[0] },
...
//     const instance = await WeddingToken.deployed();
//     const incoming = await instance.getIncomingPropositions({
//       from: accounts[1],
//     });
...
//     const instance = await WeddingToken.deployed();
//     const outgoing = await instance.getOutgoingPropositions({
//       from: accounts[0],
//     });
...
//       assert.equal(
//         new BN(outgoing[1][i].divorceTimeout).eq(
//           new BN(props[i].divorceTimeout),
//         ),
//         true,
//       );
...
//       assert.equal(
//         new BN(outgoing[1][i].tokenId).eq(new BN(props[i].tokenId)),
//         true,
//       );
...
//     const instance = await WeddingToken.deployed();
//     await truffleAssert.reverts(
//       instance.acceptProposition.sendTransaction(accounts[0], '0x', '0x', {
//         from: accounts[1],
//       }),
//       'WeddingToken: accept from partner required',
//     );
...
//     const instance = await WeddingToken.deployed();
//     const tx = await instance.acceptProposition(
//       accounts[1],
//       '0x' + sha256('new link 2'),
//       '0x' + sha256('new data 2'),
//       { from: accounts[0] },
//     );
//     await truffleAssert.eventEmitted(tx, 'Wedding', (ev) => {
//       return (
//         ev.author.toLowerCase() === accounts[0].toLowerCase() &&
...
//     const instance = await WeddingToken.deployed();
//     const tx = await instance.confirmDivorce.sendTransaction({
//       from: accounts[1],
//     });
...
//     const instance = await WeddingToken.deployed();
//     await instance.acceptProposition.sendTransaction(
//       accounts[0],
//       '0x' + sha256('some link'),
//       '0x' + sha256('some data'),
//       { from: accounts[2] },
